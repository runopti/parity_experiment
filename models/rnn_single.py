...
class RNNmodel:
    def __init__(self, seq_len, target_size, hidden_size, batch_size, loss_type):
        ...
        self.target = tf.placeholder(tf.float32, shape=(batch_size))

        self._initial_state = tf.zeros([self.batch_size, 2*self.hidden_size])
        state = self._initial_state
        self.output, self.hidden_state = self._inference(self.input, state)
        self.loss = loss = self._loss(self.output, self.target, loss_type)
    ...
    def _inference(self, input, hidden_state):
        ...
        with tf.name_scope("LSTM") as scope:
            ...
            for i in range(len(self.seq_len)):
                ...
                cell_output, hidden_state = lstm(hidden_in[:,i], hidden_state)
                ## ???? #
    ...
    def _loss(self, output, target, loss_type):
        if loss_type == "mse":
        ...
        if loss_type == "entropy":
            with tf.name_scope("entropy") as scope:
                ...
                tf.scalar_summary("entropy-loss",loss)
                return loss
