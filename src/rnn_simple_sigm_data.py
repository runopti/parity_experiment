...
# USe n_test data!!!
def calc_accuracy(test_data, state):
    total_sum = 0
    for i in range(len(test_data)):
        #state = initial_state.eval()
        x = test_data[i]
        #x = np.asarray(x).reshape(1, seq_len)
        y = getData.createTargetData(x)[-1]
        #y_target = np.zeros((1,1))
        #if y == 0: y_target[0][0] = 0.0
        #else: y_target[0][1] = 1.0
        feed_dict={initial_state: state, data:x.reshape(batch_size, seq_len)}
        output_ = session.run(output, feed_dict=feed_dict)
        #print(np.argmax(y_target))
        #print(np.argmax(output_))
        #print(y)
        #print(output_[0])
        if output_[0][0] < 0.5: ans = 0
        else: ans = 1
        if y == ans:
            total_sum += 1
...
n_test = 100
input_data, test_data = getData.createRandomData(n, n_test, seq_len)
#target_data = getData.createTargetData(input_data)

val_input_data, _ = getData.createRandomData(n,n_test, seq_len)
...
with tf.Graph().as_default():
    ...
    for i in range(seq_len):
        ...
        data_i = tf.reshape(data[:,i],[1,1])
        # data_i = tf.Print(data_i, [data_i], message="This is data_i: ")
        cell_output, state = rnn_cell(data_i, state) # should change [1,1] if I want to change batch_size?
        ...
        if args.train_method == "single":                                                
            ...
            #target_list[i] = tf.Print(target_list[i], [target_list[i]],message="This is target_list[i]: ")
    ...
    with tf.Session() as session:
        ...
        while abs(new_val_loss - old_val_loss) > args.loss_diff_eps:
            ...
            total_loss = 0
            step_count = 0
            for step in range(len(input_data)): #enumerate(reader.parity_iterator(input_data,target_data,batch_size, seq_len)):
                step_count += 1
                x = input_data[step]
                if args.train_method == "single":
                    y = getData.createTargetData(x)[-1]
                    y_target = np.float32(np.array(y).reshape(1,1))
                    #if y == 0: y_target[0][0] = 1 
                    #else: y_target[0][1] = 1 
                else:    # y_target needs to be shape=[num_seq * [batch_size, output_size]
                    y = getData.createTargetData(x)
                    y_target = np.zeros((seq_len, 1, 1))
                    for i in range(seq_len):
                        if y[i] == 0: y_target[i][0][0] = 0.0
                        else: y_target[i][0][0] = 1.0
            ...
            acc = calc_accuracy(test_data, numpy_state)
            print(acc)
            acc_list.append(acc)
            ...
            val_total_loss = 0
            num_total_steps = 0
            for step_val in range(len(val_input_data)): #enumerate(reader.parity_iterator(val_input_data,val_target_data,batch_size, seq_len)):
                val_x = val_input_data[step_val] 
                if args.train_method == "single": # using only the last target (at the end of the time_step)
                    val_y = getData.createTargetData(val_x)[-1] 
                    val_y_target = np.float32(np.array(val_y).reshape(1,1))
                    #if val_y == 0: val_y_target[0][0] = 1 
                    #else: val_y_target[0][1] = 1
                else:
                    val_y = getData.createTargetData(val_x)
                    val_y_target = np.zeros((seq_len, 1, 1))
                    for i in range(seq_len):
                        if val_y[i] == 0: val_y_target[i][0][0] = 0.0
                        else: val_y_target[i][0][0] = 1.0
                val_loss = session.run([loss], feed_dict={initial_state: numpy_state, data: val_x.reshape(batch_size,seq_len), target: val_y_target, learning_rate: 0.0})
